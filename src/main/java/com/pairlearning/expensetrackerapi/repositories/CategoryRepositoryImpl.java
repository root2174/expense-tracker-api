...
public class CategoryRepositoryImpl implements CategoryRepository{
    ...
    private static final String SQL_FIND_ALL = "SELECT C.*, COALESCE(SUM(T.AMOUNT),0) TOTAL_EXPENSE FROM " +
            "ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY_ID WHERE C.USER_ID = ? " +
            "GROUP BY C.CATEGORY_ID";
    private static final String SQL_FIND_BY_ID = "SELECT C.*, COALESCE(SUM(T.AMOUNT),0) TOTAL_EXPENSE FROM " +
            "ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY_ID " +
    ...
    private final RowMapper<Category> categoryRowMapper = ((rs, i) -> new Category(rs.getInt("CATEGORY_ID"),
            rs.getInt("USER_ID"),
            rs.getString("TITLE"),
